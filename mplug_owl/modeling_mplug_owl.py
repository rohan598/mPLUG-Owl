...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def forward(
        ...
        # print(f"loss mask shape bf: {loss_mask.shape}")
        loss_mask = loss_mask[:, :-1]
        # print(f"loss mask shape af: {loss_mask.shape}")
        loss_mask = loss_mask * non_padding_mask * non_media_mask * prompt_mask
        # print(f"labels mask shape bf: {labels.shape}")
        labels[:, 1:][loss_mask != 1] = -100
        # print(f"labels mask shape af: {labels.shape}")
        # Forward into GPT
